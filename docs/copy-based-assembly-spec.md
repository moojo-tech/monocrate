# Monodrop: Copy-Based Assembly

## Problem Statement

An earlier version of Monodrop used esbuild to bundle a package and its in-repo dependencies into a single `index.js` file. This worked for JavaScript but **produced no type declarations**. The published package had no `.d.ts` files, making it unusable for TypeScript consumers.
...
This was a breaking change to monodrop's output format. The output changed from:
```
output/
...
Consumers of monodrop needed to update any assumptions about output structure.
